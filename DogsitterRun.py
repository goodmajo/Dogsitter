...
def Html_Author(temperature, a_dog, the_lights, a_stereo):
    ...
    <header>
      <h1>Where is Olive Today?</h1>
      <h9>Spoiler: she's probably sleeping somewhere...</h9>
    </header>

    <nav class="nav">
      <img src="OliveThePitBull.jpg" alt="Olive the Pit Bull" width="337.5" height="450">
    </nav>

    <article class="article">
    <h1><font color=”#0000FF”>{off_message}</font></h1>
      <h1>Olive is:</h1>
...
def Send_Mail(temperature, sound_level, the_stereo, the_lights):
    print("Sending an email!")
    mail_server = smtplib.SMTP_SSL('smtp.gmail.com', 465)
    mail_server.ehlo()

    email_sender = 'whereisolive.sender@gmail.com'
    email_password = '0l1v3isadog'
    email_receiver = 'whereisolivetoday.receiver@gmail.com'

    email_subject = "Where is Olive - Temperature is {}.".format(temperature)
    greetings = "Hello!\n"
    temp_msg = "Temp is currently = {0} degrees F\n".format(temperature)
    sound_msg = "Sound level is currently {0} dB. The stereo is {1}.\n".format(sound_level, the_stereo.state)
    lights_msg = "The lights are {0} {1}.\n".format(the_lights.state, the_lights.on_location)
    email_text = greetings + temp_msg + sound_msg + lights_msg
