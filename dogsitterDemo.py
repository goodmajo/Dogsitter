...
dayNightSwitch = x          # This switch just toggles day mode and night mode. Only for use in demo.
...
GPIO.setup(dayNightSwitch, GPIO.IN)
...
time.sleep(10)
PrintTime("Starting Dogsitter Demo now")
...
def main():
    try:
        while True:
            # Tell system what time it is.
            if GPIO.input(dayNightSwitch == 1):
                sunIsUp = True
        ...
        PrintTime("Dogsitter Demo has been stopped by user")
        GPIO.cleanup()
        exit()
